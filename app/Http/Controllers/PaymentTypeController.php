...
class PaymentTypeController extends Controller
{
    ...
    public function edit($id) {
        $res = DB::table('payment_type')
            ...
            ->where('id', $id)
            ->first();
        return view('paymenttype.edit', compact('res') );
    }

    public function list(Request $request) {
        if ($request->ajax()) {
            $data = DB::table('payment_type');
            $data = $data->select('payment_type.*');
            $data = $data->where('active', $request->active);
            $data = $data->get();
            return DataTables::of($data)
                ->addIndexColumn()
                 ->addColumn('paymenttypename', function($row){
                    $paymenttypename = $row->name;
